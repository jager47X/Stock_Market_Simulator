//
// Created by yutto on 2/4/2024.
//

#include "BankStocks.h"


BankStocks::BankStocks(string ind, string name, double total, double own, double day_c, double val, double share) :Stock(ind, name, own, total, day_c, val, share) {
    srand(time(NULL));
    owned=0;
    change=0;
}
...
double BankStocks::stockChange() {
    ...
    double result = Stock::stockChange() + getDay_changePer();
